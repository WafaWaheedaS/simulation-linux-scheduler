...
struct prioArray{
    ...
    long avgQ;       //average Queue length before division
...
void main(){
    ...
    allbtPerCPU = malloc(sizeof(long)*numCpus);
    if(allbtPerCPU==NULL){
        printf("memory allocation error");
    }
    allatPerCPU = malloc(sizeof(long)*numCpus);
    if(allatPerCPU==NULL){
        printf("memory allocation error");
    }
    rt = malloc(sizeof(long)*numCpus);
    if(rt==NULL){
        printf("memory allocation error");
    }
    ...
    int balanceEvery = 25000; //preform load balancing after every 50 tasks
    int k=0;
    int maxQ=0;
	...
	while(!isEmptyEQ(eventsQheadPtr)){
		...
		if(currentEvent.type==1){   //Departure
			...
			if(currentEvent.task.bt != 0){
                ...
                if ( (bonus-5) >=((currentEvent.task.pr/4) - 28)){
                   // printf("interactive");
                    numInteractive++;
                }else{
                ...
                runQueue[currentEvent.task.onCpu].expiredQ.avgQ+=(simTime-runQueue[currentEvent.task.onCpu].expiredQ.prevT)*runQueue[currentEvent.task.onCpu].expiredQ.nr_active;
        ...
        if(currentEvent.type==0){   //arrival
            ...
            runQueue[i].activeQ.avgQ+=(simTime-runQueue[i].activeQ.prevT)*runQueue[i].activeQ.nr_active;
	...
	printf("\tTime Simulation Ended = %d\n", simTime);
    ...
    printf("\tnumber of interactive tasks = %d\n", numInteractive);
    printf("\tnumber of non-interactive tasks = %d\n", nonInteractive);
}//end main
...
SchedEvent serviceTask(int onCpu, SchedEvent currentEvent){
            ...
            if(newEvent.task.status==0){
                rt[onCpu]+=simTime-newEvent.task.at;
                allbtPerCPU[onCpu]+=newEvent.task.bt;
                allatPerCPU[onCpu]+=newEvent.task.at;
...
//			}else{
    ...
    if(prio != -1){
        ...
        runQueue[busy].expiredQ.avgQ+=(simTime-runQueue[busy].expiredQ.prevT)*runQueue[busy].expiredQ.nr_active;
        ...
        runQueue[busy].activeQ.avgQ+=(simTime-runQueue[busy].activeQ.prevT)*runQueue[busy].activeQ.nr_active;
